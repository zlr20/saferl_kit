...
if __name__ == "__main__":
    ...
    if not args.exp_name:
        if args.use_usl:
            ...
            logger = SafeLogger(exp_name='7_CPO',env_name=f"{args.env}",seed=args.seed,fieldnames=['EpRet','EpCost','CostRate'])
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if done: 
            if args.use_cpo:
                ...
                for _ in range(80): # this number follows the spinngup vpg
                    # train enough critic 
                    policy.train_vanilla_pg_critic(replay_buffer, args.batch_size)

                # train action for once 
                policy.train_vanilla_pg(replay_buffer, args.batch_size, runtime_logger)
