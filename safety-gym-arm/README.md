...
For a full list of configuration options, see the `Engine` [code itself](safety_gym_arm/envs/engine.py). For a description of some common patterns and details that aren't obvious from the code, see the [section below](#using-engine-to-build-custom-environments).
...
We compute normalized scores using reference statistics from our run of unconstrained PPO, with 10M env steps for environments with Point or Car robots and 100M env steps for environments with the Doggo robot. These reference statistics are available in [the bench folder](safety_gym_arm/bench/characteristic_scores.json), and we provide a [utility function](safety_gym_arm/bench/bench_utils.py#L40) to calculate normalized for an arbitrary environment.
