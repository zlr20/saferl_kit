...
from saferl_utils import Critic,Q_V_Critic, Stochastic_Actor
...
class TRPO(object):
    def __init__(
        ...
        self.cost_critic = Q_V_Critic(state_dim, action_dim).to(device)
        self.cost_critic_optimizer = torch.optim.Adam(self.cost_critic.parameters(), lr=3e-4)
        
        self.reward_critic = Q_V_Critic(state_dim, action_dim).to(device)
