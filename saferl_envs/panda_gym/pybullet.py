...
class PyBullet:
    ...
    def __init__(
        self, render: bool = False, n_substeps: int = 20, background_color: np.ndarray = np.array([255.0, 250.0, 250.0])
    ) -> None:
    ...
    def loadURDF(self, body_name: str, **kwargs: Any) -> None:
        ...
        """
        id = self.physics_client.loadURDF(**kwargs)
        self._bodies_idx[body_name] = id
    ...
    def create_table(
        ...
        self.create_box(
            ...
            specular_color=np.zeros(3),
            rgba_color=np.array([1, 0.99, 0.95, 1]),
            lateral_friction=lateral_friction,
            spinning_friction=spinning_friction,
        )
